...
#This is a crawler file to get test data of 3-point field goal, assist and points per game of players.
...
for i in range(0, len(pts)):
    if fg3_pct[i].text != "":
        liss = [player[i].text, float(fg3_pct[i].text) * 100, int(ast[i].text) / int(games[i].text), int(pts[i].text) / int(games[i].text)]
        ws.append(liss)
